...
#include <crunch/messages/crunch_messages.hpp>  // For macro
...
TEST_CASE("TLV: Packed Array Field", "[tlv]") {
    TestMessage msg;
    ...
    // Field 4: Array with 2 elements [10, 20]
    // Tag = 0x21
    payload.push_back(std::byte{0x21});
    // TotalLength = 3 (count varint + 2 value varints)
...
TEST_CASE("TLV: Array count too high (underflow)", "[tlv][error]") {
    TestMessage msg;

    // Tag(4, LengthDelimited) = 0x21
    std::vector<std::byte> payload;
    payload.push_back(std::byte{0x21});
    // TotalLength = 3
    payload.push_back(std::byte{3});
    // Count = 5 (but only 2 values provided)
...
TEST_CASE("TLV: Array length too short (truncated content)", "[tlv][error]") {
    TestMessage msg;

    // Tag(4, LengthDelimited) = 0x21
    std::vector<std::byte> payload;
    payload.push_back(std::byte{0x21});
    // TotalLength = 2 (but we need 3 bytes: count + 2 values)
    payload.push_back(std::byte{2});
    // Count = 2
    payload.push_back(std::byte{2});
...
struct MapTestMessage {
    static constexpr MessageId message_id = 1001;

    MapField<1, Int32<None>, Int32<None>, 4, None> map_field;
...
TEST_CASE("TLV: Map count too high (underflow)", "[tlv][error]") {
    MapTestMessage msg;

    // Tag(1, LengthDelimited) = 0x09
    std::vector<std::byte> payload;
    payload.push_back(std::byte{0x09});
    // TotalLength = 3
    payload.push_back(std::byte{3});
    // Count = 5 (but only 1 key-value pair provided)
    payload.push_back(std::byte{5});
    // Only 1 pair: key=1, value=2
...
TEST_CASE("TLV: Map length too short (truncated content)", "[tlv][error]") {
    MapTestMessage msg;

    // Tag(1, LengthDelimited) = 0x09
    std::vector<std::byte> payload;
    payload.push_back(std::byte{0x09});
    // TotalLength = 2 (but we need 3: count + key + value)
    payload.push_back(std::byte{2});
    // Count = 1
    payload.push_back(std::byte{1});
    // Only key, no value
