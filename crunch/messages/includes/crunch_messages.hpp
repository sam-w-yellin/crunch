...
concept tuple_members_are_valid_fields_impl =
    (ValidField<std::remove_cvref_t<std::tuple_element_t<Is, Tuple>>> && ...);

template <typename Tuple, typename IndexSeq>
struct tuple_members_are_valid_fields_helper : std::false_type {};

template <typename Tuple, std::size_t... Is>
struct tuple_members_are_valid_fields_helper<Tuple, std::index_sequence<Is...>>
    : std::bool_constant<tuple_members_are_valid_fields_impl<Tuple, Is...>> {};

template <typename Tuple>
concept tuple_members_are_valid_fields = tuple_members_are_valid_fields_helper<
    Tuple, std::make_index_sequence<std::tuple_size_v<Tuple>>>::value;
...
template <typename Tuple, typename IndexSeq>
struct tuple_members_have_unique_ids_helper;

template <typename Tuple, std::size_t... Is>
struct tuple_members_have_unique_ids_helper<Tuple, std::index_sequence<Is...>> {
    static constexpr bool value = !has_duplicates<std::remove_cvref_t<
        std::tuple_element_t<Is, Tuple>>::field_id...>::value;
};

template <typename Tuple>
concept has_unique_field_ids = tuple_members_have_unique_ids_helper<
    Tuple, std::make_index_sequence<std::tuple_size_v<Tuple>>>::value;
